...
void handle_set_nickname (char **args, unsigned count, unsigned n) {
        if(count != 2){
                char *command_name[1];
                command_name[0] = "set_nickname";
                handle_invalid_arguments(*command_name, n);
                return;
        }

        struct user_info *user = find_user(args[0]); /* REPLACE ME */
        char* temp_nickname = create_name(args[1]);
        if(isvalidname(temp_nickname) && user != NULL && temp_nickname != NULL){
                free(*(user->nickname));
                *(user->nickname) = temp_nickname;
        } else {
                char *command_name[1];
                command_name[0] = "set_nickname";
                handle_invalid_arguments(*command_name, n);
...
void handle_clear_nickname (char **args, unsigned count, unsigned n) {
	if(count != 1){
        ...
        if(user != NULL){
                ...
                *(user->nickname) = create_name(user->name_info->name);
        } else {
                char *command_name[1];
                command_name[0] = "clear_nickname";
                handle_invalid_arguments(*command_name, n);
...
void handle_mute (char **args, unsigned count, unsigned n) {
        ...
        char *name = args[0];
        struct user_info *mutedUser = find_user(name);
        bool ismute = false;

        if(mutedUser == NULL || users[n]->muted_total > users[n]->muted_capacity){
                char *command_name[1];
                command_name[0] = "mute";
                handle_invalid_arguments(*command_name, n);
                return;
        }
        if(!strcmp(name, users[n]->name_info->name)){
                char *command_name[1];
                command_name[0] = "mute";
                handle_invalid_arguments(*command_name, n);
                return;
        }
        ismute = ismuted(users[n],mutedUser);

        if(!ismute && mutedUser != NULL){
                int i = 0;
                while(i < (users[n]->muted_capacity)){
                        if((users[n]->muted)[i]->name == NULL){
                                free((users[n]->muted)[i]);
                                struct name_info* temp_mute = create_name_info(mutedUser->name_info->name);
                                (users[n]->muted)[i]= temp_mute;
                                (users[n]->muted_total)++;
                                break;
                        }
                i++;
...
void handle_unmute (char **args, unsigned count, unsigned n) {
        ...
        bool ismute = ismuted(users[n],mutedUser);
        if(!ismute){
                char *command_name[1];
                command_name[0] = "unmute";
                handle_invalid_arguments(*command_name, n);
        ...
        for(int i = 0; i < users[n]->muted_capacity; i++){
                if((users[n]->muted)[i]->name != NULL && strcmp(name,(users[n]->muted)[i]->name) == 0){
                        free((users[n]->muted)[i]->name);
                        free((users[n]->muted)[i]);
                        (users[n]->muted)[i] = (struct name_info*) malloc(sizeof(struct name_info));
                        ((users[n]->muted)[i])->name = NULL;
                        (users[n]->muted_total)--;
                }
        }
